...
public class Packeteer {
    ...
    protected static boolean handleIncoming(PacketPlayer player, Object packet) {
        ...
        for (PacketMap map : Packeteer.packetMap) {
            if (map.getHandleType().equals(PacketType.INCOMING) && (!map.getForClass().isEmpty() ? event.getPacket().getHandle().getClass().getSimpleName().equalsIgnoreCase(map.getForClass()): true)) {
                try {
    ...
    protected static boolean handleOutgoing(PacketPlayer player, Object packet) {
        ...
        PacketEvent event = new PacketEvent(p, player);
        for (PacketMap map : Packeteer.packetMap) {
            if (map.getHandleType().equals(PacketType.OUTGOING) && (!map.getForClass().isEmpty() ? event.getPacket().getHandle().getClass().getSimpleName().equalsIgnoreCase(map.getForClass()): true)) {
                try {
                    map.getMethod().invoke(map.getListener(), event);
                } catch (Exception e) {
                    e.printStackTrace();
